...
import { Scissors, Clock, ChevronRight, ChevronLeft, Check, MessageCircle, MapPin, Phone, Star, X, Calendar, User } from "lucide-react";
...
const services = [
  { id: 1, name: "Corte Clássico", desc: "Tesoura e máquina, acabamento perfeito", price: 45, duration: 30, icon: "✂" },
  { id: 2, name: "Barba Completa", desc: "Modelagem, hidratação e toalha quente", price: 35, duration: 25, icon: "🪒" },
  { id: 3, name: "Corte + Barba", desc: "O combo completo para o visual ideal", price: 75, duration: 55, icon: "⭐" },
  { id: 4, name: "Degradê", desc: "Fade perfeito nas laterais e nuca", price: 50, duration: 35, icon: "💈" },
  { id: 5, name: "Sobrancelha", desc: "Design e modelagem de sobrancelha", price: 20, duration: 15, icon: "✦" },
  { id: 6, name: "Hidratação Capilar", desc: "Tratamento profundo para os fios", price: 40, duration: 40, icon: "💧" },
];
...
const s = {
  ...
  header: { display: "flex", alignItems: "center", gap: 12, padding: "16px 20px", borderBottom: `1px solid ${DARK4}`, background: DARK },
  ...
  waBtn: { position: "fixed", bottom: 24, right: 20, width: 54, height: 54, borderRadius: "50%", background: "#25D366", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", boxShadow: "0 4px 14px rgba(37,211,102,0.35)" },
...
function WaBtn() {
  return (
    <a href="https://wa.me/5511999999999?text=Ol%C3%A1!%20Gostaria%20de%20agendar%20um%20hor%C3%A1rio." target="_blank" rel="noreferrer" style={{ ...s.waBtn, textDecoration: "none" }}>
      ...
      <div style={s.hero}>
        <div style={s.logo}>
          ...
          <div style={{ marginLeft: "auto" }}>
            <div style={s.badge}><span style={s.dot}></span>Aberto</div>
          </div>
        </div>
        <div style={{ background: DARK3, borderRadius: 16, padding: 20, border: `1px solid ${DARK4}` }}>
          <div style={{ fontSize: 13, color: GOLD, fontWeight: 600, marginBottom: 4 }}>Bem-vindo à</div>
        ...
        {services.map(svc => (
          <div key={svc.id} style={s.serviceCard} onClick={onBook}>
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={s.svcMeta}><Clock size={11} /> {svc.duration} min</div>
            </div>
            <div style={{ textAlign: "right" }}>
              <div style={s.svcPrice}>R$ {svc.price}</div>
...
function StepService({ selected, onSelect, onNext, onBack }) {
  return (
    <div>
      <div style={s.header}>
...
function StepConfirm({ service, barber, day, time, onConfirm, onBack }) {
  return (
    <div>
      <div style={s.header}>
        <div style={s.backBtn} onClick={onBack}><ChevronLeft size={18} color={TEXT} /></div>
      ...
      <div style={s.confirmCard}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          ...
          <div style={{ fontSize: 22, fontWeight: 700, color: GOLD }}>R$ {service.price}</div>
        </div>
      </div>
      <div style={{ padding: "0 20px" }}>
        <button style={s.btn} onClick={onConfirm}>Confirmar Agendamento</button>
        <button style={s.btnOutline} onClick={onBack}>Voltar e Editar</button>
      </div>
    </div>
  );
}
    ...
    <div style={{ padding: "48px 20px 20px", textAlign: "center" }}>
      ...
      <div style={{ ...s.confirmCard, textAlign: "left" }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between" }}>
          ...
          <span style={{ fontSize: 17, fontWeight: 700, color: GOLD }}>R$ {service.price}</span>
        </div>
      </div>
...
export default function App() {
  ...
  return (
    <div style={s.app}>
      {screen === "home" && <HomeScreen onBook={startBook} />}
      {screen === "book" && step === 1 && <StepService selected={service} onSelect={setService} onNext={() => setStep(2)} onBack={goHome} />}
      ...
      {screen === "book" && step === 4 && <StepConfirm service={service} barber={barber} day={day} time={time} onConfirm={() => setStep(5)} onBack={() => setStep(3)} />}
